...
const error_email_dosen = computed(() => {
    if (!formDosen.value.email.trim()) {
        return 'Email tidak boleh kosong';
    } else if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(formDosen.value.email)) {
        return 'Email tidak valid';
    } else {
        return '';
    }
...
const error_password_dosen = computed(() => {
    if (!formDosen.value.password.trim()) {
        return 'Password tidak boleh kosong';
    } else if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/.test(formDosen.value.password)) {
        return 'Password harus memiliki setidaknya satu huruf besar, satu huruf kecil, dan satu angka';
    } else {
        return '';
    }
    ...
    <div
        ...
        <div class="max-w-md w-full">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-neutral-200 p-8">
                <div class="mb-6">
                    <label class="text-sm text-neutral-700 mb-2 block">Daftar sebagai</label>
                    <div class="relative w-full bg-neutral-100 rounded-full p-1 flex border border-neutral-200">
                        <div v-if="toggleRegisterUser === 'Mahasiswa'" id="toggle-bg"
                        ...
                        <button type="button" @click="toggel_mahasiswa"
                            class="relative z-10 w-1/2 py-2 text-sm transition-colors duration-300 cursor-pointer"
                            :class="{ 'text-neutral-900': toggleRegisterUser === 'Mahasiswa', 'text-neutral-500': toggleRegisterUser === 'Dosen' }">
                            Mahasiswa
                        </button>
                        <div v-if="toggleRegisterUser === 'Dosen'" id="toggle-bg"
                            ...
                            style="transform: translateX(0%);"></div>
                        <button type="button" @click="toggel_dosen"
                            class="relative z-10 w-1/2 py-2 text-sm transition-colors duration-300 cursor-pointer"
                            :class="{ 'text-neutral-900': toggleRegisterUser === 'Dosen', 'text-neutral-500': toggleRegisterUser === 'Mahasiswa' }">
                            Dosen
                        </button>
                    </div>
                </div>
                <!-- form mahasiswa -->
                <form v-if="toggleRegisterUser === 'Mahasiswa'" @submit.prevent="register" class="space-y-6">
                    <div class="space-y-2">
                ...
                <!-- form dosen -->
                <form v-if="toggleRegisterUser === 'Dosen'" @submit.prevent="registerDosen" class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-sm text-neutral-700" for="role_prodi">Prodi</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 pb-[15px] flex items-center pointer-events-none">
                            ...
                            <select v-model="formDosen.prodi_id" id="role_prodi"
                                class="block w-full pl-12 pr-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white appearance-none">
                                <option>Pilih Prodi Anda</option>
                                <option v-for="prodi in dataProdi" :key="prodi.id" :value="prodi.id">{{
                                    prodi.nama_prodi }}</option>
                            ...
                            <p v-if="error_prodi_dosen" class="text-xs text-red-500">{{ error_prodi_dosen }}</p>
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm text-neutral-700" for="name">Nama Lengkap</label>
                        <div class="relative">
                            ...
                            <input v-model="formDosen.nama" type="text" id="name"
                                class="block w-full pl-12 pr-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white"
                                placeholder="Masukkan nama lengkap Anda">
                            <p v-if="error_nama_dosen" class="text-xs text-red-500">{{ error_nama_dosen }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="formDosen.email" type="email" id="email"
                                class="block w-full pl-12 pr-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white"
                                placeholder="Masukkan alamat email Anda">
                            <p v-if="error_email_dosen" class="text-xs text-red-500">{{ error_email_dosen }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="formDosen.nip" type="text" id="nim"
                                class="block w-full pl-12 pr-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white"
                                placeholder="Masukkan Nomor Induk Pegawai">
                        </div>
                        <p v-if="error_nip_dosen" class="text-xs text-red-500">{{ error_nip_dosen }}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <select v-model="formDosen.role" id="role_peran"
                                class="block w-full pl-12 pr-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white appearance-none">
                                <option value="">Pilih peran Anda</option>
                            ...
                            <p v-if="error_role_dosen" class="text-xs text-red-500">{{ error_role_dosen }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="formDosen.username" type="text" id="username"
                                class="block w-full pl-12 pr-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white"
                                placeholder="User name anda">
                            <p v-if="error_username_dosen" class="text-xs text-red-500">{{ error_username_dosen }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="formDosen.password" :type="typePassword ? 'password' : 'text'" id="password"
                                class="block w-full pl-12 pr-12 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-all duration-200 bg-neutral-50 focus:bg-white"
                                placeholder="Buat password">
                            <div class="absolute inset-y-0 right-0 pr-4 pb-[15px] flex items-center">
                                <!-- <i class="fa-regular fa-eye text-neutral-400 cursor-pointer hover:text-neutral-600 transition-colors"></i> -->
                            ...
                            <p v-if="error_password_dosen" class="text-xs text-red-500">{{ error_password_dosen }}</p>
                        </div>
                    </div>
